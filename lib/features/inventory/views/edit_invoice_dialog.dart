...
class _EditInvoiceDialogState extends State<EditInvoiceDialog> {
  ...
  void initState() {
    ...
    _invoiceDateController = TextEditingController(
      text: formatDate(widget.invoice.date),
    );
    selectedCompanyId = widget.invoice.companyId;
  ...
  Future<void> _submitUpdate() async {
    ...
    // Update in database
    await DatabaseHelper().updateInvoice(updatedInvoice);
    // Reload invoices in controller
    controller.loadInvoices();
    // Update ledger entries - delete old ones and add new ones
    final LedgerController ledgerController = Get.find<LedgerController>();
    // Delete old ledger entries for this invoice
    await ledgerController.deleteLedgerEntriesByReference(
      widget.invoice.reference,
    );
    // Add new ledger entries for each product
    for (var item in selectedItems) {
      LedgerEntry entry = LedgerEntry(
        ...
        date: invoiceDate,
        companyId: selectedCompanyId,
        referenceNo: updatedInvoice.reference,
        qty: item['qty'] as int,
